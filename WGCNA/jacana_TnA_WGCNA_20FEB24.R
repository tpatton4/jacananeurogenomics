...
POA1 = read.csv("jacanaTnA_counts_8FEB24.csv")
POA <- subset(POA1, select=-c(Gene.name, GSF2960.NOF6.TnA)) #remove gene names and outlier
...
datTrait <- datTrait[!datTrait$Individual == "GSF2960.NOF6.TnA", ] #get rid of NOM4, an outlier
...
MEs = orderMEs(MEs0)
write.csv(MEs,file="jacana_TnA_ModuleEigengenes_20FEB24.csv")
...
# Export the netLwork into edge and node list files Cytoscape can read
modules = c("lightgreen")
cyt = exportNetworkToCytoscape(modTOM, edgeFile = paste("CytoscapeInputTnA-edges-", paste(modules, collapse="-"), ".txt", sep=""), nodeFile = paste("CytoscapeInputPOA-nodes-", paste(modules, collapse="-"), ".txt", sep=""),
                               weighted = TRUE,
                               threshold = 0.02,
                               nodeNames = modProbes,
                               altNodeNames = modGenes,
