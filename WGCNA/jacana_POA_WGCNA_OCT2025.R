...
library(tidyverse)
datTrait = subset(traitData, select=-c(Individual_Region, Testosterone,log.T., Male.Breeding.Stage, Swoops, WingSpread, Flyovers, Facial.Shield.Length, Bill.length, Bill.width, Wing.length, Group, Julian.Collected, Brain.Time, Euthanize.Time, Vocalizations, Avg.Testis.Mass, Ovary.mass))
dim(datTrait) # 24 `12`
...
MEs = orderMEs(MEs0)
write.csv(MEs,file="jacana_POA_ModuleEigengenes_08SEPT24.csv")
...
Mass_min = as.data.frame(datTraits1$Avg.Spur);
...
verboseScatterplot(abs(geneModuleMembership[moduleGenes, column]),
                   ...
                   ylab = "Gene significance for spur",
...
TOM = TOMsimilarityFromExpr(datExpr1, power = 14)
#Select module probes 
