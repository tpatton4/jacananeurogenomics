...
POA1 = read.csv("jacana_counts_genenames_POA_8FEB24.csv")
...
traitData <-  read.csv("jacana_POA_traitdata_OCT25.csv")
traitData <- data.frame(traitData, row.names=traitData[,1])
traitData <- traitData[-1] #remove names since they're colnames now
traitData <- traitData[row.names(traitData) != "GSF2960.NOM4.POA", ] #remove F6 outlier
...
POAdds <- DESeqDataSetFromMatrix(
  countData=countMatrixFiltered, 
  colData= traitData,
  design= ~ Group)

POAdds
POAvsd <- vst(POAdds, blind = FALSE) #blind = FALSE bc we will use experimental design info to estimate dispersion trend
POAvst_counts <- assay(POAvsd)

# Check how well normalization worked
rld <- rlog(POAdds, blind = FALSE)
dds <- estimateSizeFactors(POAdds)

df <- bind_rows(
  as_data_frame(log2(counts(dds, normalized=TRUE)[, 1:2]+1)) %>%
    mutate(transformation = "log2(x + 1)"),
  as_data_frame(assay(POAvsd)[, 1:2]) %>% mutate(transformation = "vst"),
...
#### Read in trait data #####
traitData = read.csv("jacana_POA_traitdata_OCT25.csv");
traitData <- traitData[traitData$Individual != "GSF2960.NOM4.POA", ] #get rid of NOM4, an outlier
dim(traitData) # 22 26
names(traitData)
...
# For main MS heatmap
datTrait = subset(traitData, select=-c(Group, Individual_Region, Testosterone,log.T., Swoops, Flyovers, Facial.Shield.Length, Bill.length, Bill.width, Facial.Shield.Width, Wing.length, Julian.Collected, Brain.Time, Euthanize.Time, Vocalizations, Avg.Testis.Mass, Ovary.mass))

# For supplementary heatmap
# datTrait = subset(traitData, select=-c(Individual_Region, Group, Testosterone, Facial.Shield.Length, Facial.Shield.Width, Bill.length, Bill.width, Wing.length))

dim(datTrait) # 23 8
names(datTrait) #these are the traits that will show up in the heatmap
datTrait[,1] #individuals for which we have trait data
datTrait <- datTrait[!datTrait$Individual == "GSF2960.NOM4.POA", ] #get rid of NOM4, an outlier
...
datTrait = subset(datTrait, select=c(Individual, Sex, Male.Breeding.Stage, Distance, WingSpread, PCAtrait))
...
#If you didn't remove NOM4 previously, you'll see that NOM4 is an outlier. Go back and remove him
...
MEs = orderMEs(MEs0)
#write.csv(MEs,file="jacana_POA_ModuleEigengenes_06NOV25.csv")
...
labeledHeatmap(Matrix = moduleTraitCor,
               ...
               colorLabels = FALSE,
               colors = palette,
               textMatrix = NULL, # can change to NULL to remove text
               setStdMargins = FALSE,
...
table(moduleColors)
#black       brown        cyan        grey   lightcyan saddlebrown     skyblue         tan      violet 
#4930         540         121        1802         550          76          79         938          44 
 


# Female-specific heatmap
traitData_females <-  read.csv("jacana_POA_WGCNATraitData_females.csv")
head(traitData_females)

# For heatmap in main text
#datTraits = subset(traitData, select=-c(Individual, Male.Breeding.Stage,Sex,Group,Avg.Testis.Mass, Julian.Collected, Brain.Time, Euthanize.Time, Individual_Region, Testosterone, Bill.length, Bill.width, Facial.Shield.Length, Facial.Shield.Width, Wing.length,Mass_min, Tarsus.length, Avg.Spur, Avg.Testis.Mass))
datTraits_females = subset(traitData_females, select=-c(Distance, Swoops, WingSpread, Vocalizations, Ovary.mass, PCAtrait, Individual, Male.Breeding.Stage,Sex,Group,Avg.Testis.Mass, Julian.Collected, Brain.Time, Euthanize.Time, Individual_Region, Testosterone, Bill.length, Bill.width, Facial.Shield.Length, Facial.Shield.Width, Wing.length,Mass_min, Tarsus.length, Avg.Spur, Avg.Testis.Mass))
head(datTraits_females)

# Recalculate MEs with color labels
MEs0 = moduleEigengenes(datExpr0, moduleColors)$eigengenes
MEs = orderMEs(MEs0)

moduleTraitCor = bicor(MEs, datTraits_females, use = "p",robustY = FALSE,maxPOutliers =0.1);
moduleTraitPvalue = corPvalueStudent(moduleTraitCor, nSamples);
#sizeGrWindow(10,6)
# Will display correlations and their p-values
textMatrix = paste(signif(moduleTraitCor, 2), "\n(",
...
palette <- colorRampPalette(c("deepskyblue3","white","coral"))(50)
labeledHeatmap(Matrix = moduleTraitCor,
               xLabels = names(datTraits_females),
               yLabels = names(MEs),
               ySymbols = names(MEs),
               colorLabels = FALSE,
               colors = palette,
...
traitData_males <-  read.csv("jacana_TnA_WGCNATraitData_males.csv")

# For heatmap in main text
datTraits_males = subset(traitData_males, select=-c(Ovary.mass,Individual,Sex,Group, Julian.Collected, Brain.Time, Euthanize.Time, Individual_Region, Testosterone, Bill.length, Bill.width, Facial.Shield.Length, Facial.Shield.Width, Wing.length,Mass_min, Tarsus.length, Avg.Spur))
head(datTraits_males)

# Recalculate MEs with color labels
MEs0 = moduleEigengenes(datExpr0, moduleColors)$eigengenes
MEs = orderMEs(MEs0)

moduleTraitCor = bicor(MEs, datTraits_males, use = "p",robustY = FALSE,maxPOutliers =0.1);
moduleTraitPvalue = corPvalueStudent(moduleTraitCor, nSamples);
#sizeGrWindow(10,6)
# Will display correlations and their p-values
textMatrix = paste(signif(moduleTraitCor, 2), "\n(",
...
Distance = as.data.frame(datTraits1$Distance);
...
modOrder = order(-abs(cor(MEs, Distance, use = "p")))
...
write.csv(geneInfo, file = "jacana_POA_WGCNAmodule_6Nov25.csv")




#focus on another trait - brown module and wingspreads
# Define variable weight containing the weight column of datTrait
PCAtrait = as.data.frame(datTraits1$PCAtrait);
...
write.csv(geneInfo, file = "jacana_POA_WGCNAmodule_6Nov25_PCAtrait.csv")
