#trying to run DESeq2 with all 3 treatment groups in the POA

library(DESeq2)
#read in gene count data
setwd("C:/Users/tmp15/Downloads/R/jacana")
counts_genes <- as.matrix(read.csv("jacanaPOA_counts_8FEB24.csv",row.names="Gene.stable.ID"))
...
#remove TNA info
treat <- treat[!(treat$Brain.Region=="TnA"),]
...
write.csv(sigcountsFvMP, "jacanaPOA_FvMP_DESeq2Out_16FEB24.csv")
write.csv(sigcountsFvMC, "jacanaPOA_FvMC_DESeq2Out_16FEB24.csv")
write.csv(sigcountsMCvMP, "jacanaPOA_MCvMP_DESeq2Out_16FEB24.csv")
...
####Volcano Plots####
with(allgenesFvMP, plot(log2FoldChange, -log10(padj), pch=19, main="Females vs Parenting Males POA", cex=0.7, xlab=bquote(~Log[2]~fold~change), ylab=bquote(~-log[10]~P~value)))
...
#FvMC
with(allgenesFvMC, plot(log2FoldChange, -log10(padj), pch=19, main="Females vs Courting Males POA", cex=0.7, xlab=bquote(~Log[2]~fold~change), ylab=bquote(~-log[10]~P~value)))
...
#MCvMP
with(allgenesMCvMP, plot(log2FoldChange, -log10(padj), pch=19, main="Courting vs Parenting Males POA", cex=0.7, xlab=bquote(~Log[2]~fold~change), ylab=bquote(~-log[10]~P~value)))
