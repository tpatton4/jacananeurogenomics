...
#remove POA info and Parentings Males
stage <- stage1[!(stage1$Brain.Region=="POA" | stage1$SexStage=="MaleP"),]
...
dds2<-DESeq(dds2)
res <- results(dds2, alpha=0.05, contrast=c("SexStage", "Female", "MaleC"))
summary(res)

#write a file with DE data for all genes
write.csv(res, "jacana_TNA_FvMC_DESeq2allGenes.csv")
#add the gene names
annot <- read.csv("counts_with_egenes_TnA.csv")
rownames(annot) <- annot[,2]
allgenes <- read.csv("jacana_TNA_FvMC_DESeq2allGenes.csv", row.names="X")
genematrix <- merge(allgenes, annot, by="row.names", all=FALSE)
head(genematrix)
write.csv(genematrix, "jacana_TNA_FvMC_DESeq2allGenes_annot_23JUN21.csv")

#looking at just the significant genes
res_sig <- subset(res,padj <0.05)
head(res_sig)
write.csv(res_sig, "jacana_TnA_FvMC_siggenes_13JUN23.csv")
...
deg<-read.csv("jacana_TnA_FvMC_siggenes_13JUN23.csv", na.strings=c("","NA"))
rownames(deg) <- deg[,1]
annot <- read.csv("counts_with_egenes_TnA.csv")
rownames(annot) <- annot[,2]
...
with(res, plot(log2FoldChange, -log10(padj), pch=19, main="Females vs Courting Males TnA", cex=0.7, xlab=bquote(~Log[2]~fold~change), ylab=bquote(~-log[10]~P~value)))

write.csv(deg, "jacana_sigcounts_TnA_FvMC_13JUN21.csv")
