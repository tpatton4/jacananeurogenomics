...
#remove TNA info and Courting Males
stage <- stage1[!(stage1$Brain.Region=="TnA" | stage1$SexStage=="MaleC"),]
...
write.csv(res, "jacana_POA_FvMP_DESeq2allGenes.csv")
#add the gene names
annot <- read.csv("counts_with_egenes_POA.csv")
rownames(annot) <- annot[,2]
allgenes <- read.csv("jacana_POA_FvMP_DESeq2allGenes.csv", row.names="X")
genematrix <- merge(allgenes, annot, by="row.names", all=FALSE)
head(genematrix)
write.csv(genematrix, "jacana_POA_FvMP_DESeq2allGenes_annot_23JUN21.csv")

#looking at just the significant genes
res_sig <- subset(res,padj <0.05)
head(res_sig)
write.csv(res_sig, "jacana_POA_FvMP_siggenes_13JUN23.csv")
...
head(sigcounts)
#write.csv(sigcounts, "jacana_sigcounts_POA_FvMP.csv")

deg<-read.csv("jacana_POA_FvMP_siggenes_13JUN23.csv", na.strings=c("","NA"))
rownames(deg) <- deg[,1]
annot <- read.csv("counts_with_egenes_POA.csv")
rownames(annot) <- annot[,2]
...
with(deg, plot(log2FoldChange, -log10(padj), pch=19, main="Females vs Parenting Males POA", cex=0.7, xlab=bquote(~Log[2]~fold~change), ylab=bquote(~-log[10]~P~value)))

write.csv(deg, "jacana_sigcounts_POA_FvMP_13JUN21.csv")
