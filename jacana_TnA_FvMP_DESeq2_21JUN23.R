##### females vs parenting males
##### TnA

library(DESeq2)
sessionInfo()
#read in gene count data
counts <- as.matrix(read.csv("counts_with_egenes_TNA.csv",row.names="Gene.stable.ID"))
...
#remove POA info and Courting Males
stage <- stage1[!(stage1$Brain.Region=="POA" | stage1$SexStage=="MaleC"),]
stage <- subset(stage,select=-c(Sex,Stage,Brain.Region))

#remove those Courting Male individuals from counts
...
dds2<-DESeq(dds2)
res <- results(dds2, alpha=0.05, contrast=c("SexStage", "Female", "MaleP"))
summary(res)

#write a file with DE data for all genes
write.csv(res, "jacana_TnA_FvMP_DESeq2allGenes.csv")
#add the gene names
annot <- read.csv("counts_with_egenes_TnA.csv")
rownames(annot) <- annot[,2]
allgenes <- read.csv("jacana_TNA_FvMP_DESeq2allGenes.csv", row.names="X")
genematrix <- merge(allgenes, annot, by="row.names", all=FALSE)
head(genematrix)
write.csv(genematrix, "jacana_TNA_FvMP_DESeq2allGenes_annot_23JUN21.csv")

#looking at just the significant genes
res_sig <- subset(res,padj <0.05)
head(res_sig)
write.csv(res_sig, "jacana_TnA_FvMP_siggenes_20JUN23.csv")
...
allcounts <- as.matrix(read.csv("counts_with_egenes_TnA.csv", row.names="Gene.stable.ID"))
...
head(sigcounts)
#write.csv(sigcounts, "jacana_sigcounts_TnA_FvMP1.csv")

deg<-read.csv("jacana_TnA_FvMP_siggenes_20JUN23.csv", na.strings=c("","NA"))
rownames(deg) <- deg[,1]
annot <- read.csv("counts_with_egenes_TnA.csv")
rownames(annot) <- annot[,2]
...
with(res, plot(log2FoldChange, -log10(padj), pch=19, main="Females vs Parenting Males TnA", cex=0.7, xlab=bquote(~Log[2]~fold~change), ylab=bquote(~-log[10]~P~value)))

write.csv(deg, "jacana_sigcounts_TnA_FvMP_13JUN21.csv")
