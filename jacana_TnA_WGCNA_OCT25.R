...
TNA1 = read.csv("jacanaTnA_counts_8FEB24.csv")
TNA <- subset(TNA1, select=-c(Gene.name, GSF2960.NOF6.TnA)) #remove gene names and outlier
...
#remove low counts
use = rowSums(TNA_normcounts > 10) >=5 # 5 individuals must have at least a normalized count of 10
...
library(tidyverse)
datTrait = subset(traitData, select=-c(Group, Male.Breeding.Stage, log.T., Individual_Region, Testosterone, Swoops, WingSpread, Flyovers, Facial.Shield.Length, Bill.length, Bill.width, Wing.length, Julian.Collected, Brain.Time, Euthanize.Time))
dim(datTrait) # 24 `16`
names(datTrait) #these are the traits that will show up in the heatmap
datTrait[,1] #individuals for which we have trait data
datTrait <- datTrait[!datTrait$Individual == "GSF2960.NOF6.TnA", ] #get rid of NOF6, an outlier

#### One morphology variable
sizemorphtest <- select(datTrait, Mass_min, Tarsus.length, Avg.Spur, Facial.Shield.Width)
...
datTraits <- data.matrix(datTraits1, rownames.force=NA)
...
MEs = orderMEs(MEs0)
write.csv(MEs,file="jacana_TnA_ModuleEigengenes_08SEPT24.csv")
...
Mass_min = as.data.frame(datTraits1$Facial.Shield.Width);
names(Mass_min) = "Facial Shield"
...
verboseScatterplot(abs(geneModuleMembership[moduleGenes, column]),
                   ...
                   ylab = "Gene significance for facial shield",
...
write.csv(geneInfo, file = "jacanaTnA_WGCNAmodule_20FEB24.csv")
...
datExpr1 <- t(datExpr) #transpose so Ensemble IDs are column names
TOM = TOMsimilarityFromExpr(datExpr1, power = 12)
#Select module probes 
...
annot <- read.csv("jacana_TnA_geneAnnotations.csv")
...
# Export the netLwork into edge and node list files Cytoscape can read
modules = c("skyblue3")
cyt = exportNetworkToCytoscape(modTOM, edgeFile = paste("CytoscapeInputTnA-edges-", paste(modules, collapse="-"), ".txt", sep=""), nodeFile = paste("CytoscapeInputTnA-nodes-", paste(modules, collapse="-"), ".txt", sep=""),
                               weighted = TRUE,
                               threshold = 0.02,
                               nodeNames = modProbes,
                               altNodeNames = modGenes,
...
#save Rdat
save.image(file="TnA_WGCNA_20FEB24.RData")


#### Eigengene plots ####
#focus on one trait
# Define variable weight containing the weight column of datTrait
Mass_min = as.data.frame(datTrait1$Mass_min);
...
names(geneModuleMembership) = paste("MM", modNames, sep="");
names(MMPvalue) = paste("p.MM", modNames, sep="");
geneTraitSignificance = as.data.frame(bicor(datExpr1, Mass_min, use = "p",robustY = FALSE,maxPOutliers =0.1));
